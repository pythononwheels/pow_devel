#
# standard pow config file
# you can derive dedicated configurations for DEV, TEST, PROD
# by just overwriting the seetings you want to change in a dedicated config file.
# see config_dev.py for an example.
#
...
server_settings = {
    ...
    "login_url"         :   "/login",
    "xsrf_cookies"      :   False,
    #"log_function"      :   you can reference your own log function here.
...
myapp = {
    ...
    "page_size"         :   5,          # pagination default
    "enable_auth"       :   False,      # False, simple or custom
    "sql_auto_schema"   :   True,       # enables DB schema reflection if no schema is given.
    ...
    "list_separator"    :   " ",        #default separator used to separate list elements send in PUT/POST requests
    "pwhash_method"     :   "pbkdf2:sha256",    # see: http://werkzeug.pocoo.org/docs/0.14/utils/#module-werkzeug.security
    "simple_conversion":   True                 # see: decoders.py-> function: pow_init_from_dict_deserializer     
    #"environment"      :   "development"       # set the current environment (also see the db section)
}

database = {
    "default_values": {     # default values set in pow sql models when no value is given
        "string"    :   "",
        "integer"   :   0,
        "float"     :   0.0,
        "list"      :   [],
    ...
    "sql"   : {
        ...
        "yield_per"     :   100,  # see: https://docs.sqlalchemy.org/en/14/orm/query.html#sqlalchemy.orm.Query.yield_per
        #
...
if os.environ.get("POW_ENV") is not None:
    # use the env var (DEV, TEST, PROD) to override the standard values
    # with the specific env one from config_dev, config_test or config_prod
    #print(os.environ.get("POW_ENV"))
    import importlib
    module_name = str.lower("config_" + os.environ.get("POW_ENV"))
    importlib.import_module("conf." + module_name)
    #print(f"server_settings:port = {server_settings['port']}")