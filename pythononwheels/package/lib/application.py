...
#
formatter = cfg.myapp["logformat"]
log_handler = logging.FileHandler(os.path.abspath(os.path.normpath(cfg.myapp["logfile"])))
...
class Application(tornado.web.Application):
    ...
    def log_request(self, handler, message=None):
        ...
        log_method("%s %d %s %.2fms %s", handler.request.remote_ip, handler.get_status(),
                handler._request_summary(), request_time, datetime.datetime.utcnow().strftime(cfg.myapp["datetime_format"]) 
            )
        if message:
            log_method("%s %d %s %s", 
                ...
                handler.get_status(), 
                str(message), 
                datetime.datetime.utcnow().strftime(cfg.myapp["datetime_format"])  
            )
    ...
    def add_rest_routes(self, route, api=None, pos=-1):
        ...
        def decorator(cls):
            ...
            ID_PATTERN = cfg.cfg.myapp["id_pattern"]
            routes = [
                ...
                ( r"/" + action_part + r"/?", { "get" : cfg.cfg.myapp["default_rest_route"], "post" : "create", "put" : "update", "delete" : "destroy" })                
            ]
            routes.reverse()
