...
def generate_app(
    ...
    elif view_type == "api-only":
        skip_dirs.extend(["static", "views"])
        filelist = [
            l
            for l in os.listdir(os.path.join(root, "stubs"))
            if str.startswith(l, "scaffold")
        ]
        filelist += [
            l
            for l in os.listdir(os.path.join(root, "stubs"))
    ...
    if sys.platform == "win32":
        ...
        sqlite_path = (
            ...
            + sys.platform
            + "). Please set sqlite connection string yourself accordingly"
        )
        ...
        for f in files:
            ...
            if not f in exclude_files:
                ...
                if path.parts[-1] in skip_dirs:
                    ...
                    if (file_extension in template_exts) and not (
                        path.parts[-1] in exclude_dirs
                    ):
                        copy_or_pump(
    ...
    #
    if view_type and view_type != "api-only":
        print(50 * "-")
        if view_type == "bs4":
        ...
        if view_type == "bs4":
            ...
            rename_extensions(
                folder, "." + view_type, ".tmpl", files=["index", "error", "404"]
            )
        ...
        if view_type != "api-only":
            print("Error: viewtype not set and apparantly no Default set either!")
        else:
            print(
                "API Only: Created a headless API only version (No views, css, js or static files included)"
            )
...
def rename_extensions(folder, old_ext, new_ext, files=None):
    ...
    for filename in os.listdir(folder):
        ...
        if files and oldbase not in files:
            continue
...
def main():
    ...
    parser.add_argument(
        "-n", "--name", action="store", dest="name", help="-n appname", required=True
    )

    parser.add_argument(
        "-p", "--path", action="store", dest="path", help="-p out_path", required=True
    )
    ...
    parser.add_argument(
        ...
        dest="view_type",
        default="bs4",
        help="set the default view framework. (spectre.css = spc || semanticui = sui || bootstrap 4 = bs4 (default) || bootstrap 5 = bs5 || api-oonly)",
    )
